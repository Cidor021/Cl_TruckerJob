...
RegisterNetEvent('CI_TruckJob:jm', function ()
    lib.registerContext({
        id = 'jobmenu',
        title = _tr('dutytitle'),
        options = {
          {
            ...
            onSelect = function ()
                if not tookVehicle then
                    ...
                    TriggerServerEvent('CI_TruckJob:gp',earned)
                else
...
RegisterNetEvent('CI_TruckJob:iv', function()
    DoScreenFadeOut(800)
...
RegisterNetEvent("CI_TruckJob:pv", function ()
    DoScreenFadeOut(800)
    while not IsScreenFadedOut() do
        Wait(100)
    end
